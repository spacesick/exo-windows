...
async def get_macos_interface_info(ip_addr: str) -> Optional[Tuple[str, str]]:
  try:
    output = await asyncio.get_running_loop().run_in_executor(subprocess_pool, lambda: subprocess.check_output(["ifconfig"]).decode("utf-8"))
...
async def get_linux_interface_info(ip_addr: str) -> Optional[Tuple[str, str]]:
  try:
    output = await asyncio.get_running_loop().run_in_executor(subprocess_pool, lambda: subprocess.check_output(["ip", "addr"]).decode("utf-8"))
...
async def get_windows_interface_info(ip_addr: str) -> Optional[Tuple[str, str]]:
  try:
    output = await asyncio.get_running_loop().run_in_executor(subprocess_pool, lambda: subprocess.check_output(["ipconfig", "/all"], universal_newlines=True))
