...
class Transformer2D(nn.Module):
  ...
  def __init__(
    ...
    self.transformer_blocks = [TransformerBlock(model_dims, num_heads, memory_dims=encoder_dims) for i in range(num_layers)]
...
class UNetModel(nn.Module):
  ...
  def __init__(self, config: UNetConfig, shard: Shard):
    ...
    block_channels = ([config.block_out_channels[0]] + list(config.block_out_channels) + [config.block_out_channels[-1]])
