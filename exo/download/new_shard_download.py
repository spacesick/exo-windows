...
def calculate_repo_progress(shard: Shard, repo_id: str, revision: str, file_progress: Dict[str, RepoFileProgressEvent], all_start_time: float) -> RepoProgressEvent:
  ...
  return RepoProgressEvent(
    shard, repo_id, revision, len([p for p in file_progress.values() if p.downloaded == p.total]), len(file_progress), all_downloaded_bytes, all_downloaded_bytes_this_session, all_total_bytes,
    all_speed, all_eta, file_progress, status
  )
      ...
      for f in files:
        if f.endswith(('.safetensors', '.bin', '.pt', '.gguf', '.json')):
          ...
          file_progress[rel_path] = RepoFileProgressEvent(
            repo_id=repo_id, repo_revision="local", file_path=rel_path, downloaded=size, downloaded_this_session=0, total=size, speed=0, eta=timedelta(0), status="complete", start_time=time.time()
          )
          total_files += 1
          total_bytes += size
  else:
...
async def download_shard(
  ...
  def on_progress_wrapper(file: dict, curr_bytes: int, total_bytes: int):
    ...
    file_progress[
      file["path"]
    ] = RepoFileProgressEvent(repo_id, revision, file["path"], curr_bytes, downloaded_this_session, total_bytes, speed, eta, "complete" if curr_bytes == total_bytes else "in_progress", start_time)
  ...
  for file in filtered_file_list:
    ...
    file_progress[
      file["path"]
    ] = RepoFileProgressEvent(repo_id, revision, file["path"], downloaded_bytes, 0, file["size"], 0, timedelta(0), "complete" if downloaded_bytes == file["size"] else "not_started", time.time())
...
class NewShardDownloader(ShardDownloader):
  ...
  async def get_shard_download_status(self, inference_engine_name: str) -> AsyncIterator[tuple[Path, RepoProgressEvent]]:
    ...
    tasks = [
      download_shard(build_full_shard(model_id, inference_engine_name), inference_engine_name, self.on_progress, skip_download=True) for model_id in get_supported_models([[inference_engine_name]])
    ]
